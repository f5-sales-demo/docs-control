...
write_translation() {
  ...
  printf '%s\n' '---' "title: Example" 'i18n:' "  sourceHash: \"$hash\"" \
    ...
    'Bare https://example.com/bare' >"$repo/$target"
}
...
setup_repo() {
  ...
  printf '%s\n' '---' 'title: Example' '---' '' '# Example' '' \
    'Keep `literal` and <Callout type="note"> unchanged.' '' \
    'Visit `https://example.com/docs`.' '' \
    'Bare https://example.com/bare' >"$repo/$source"
...
for source, expected_hash in requests:
    ...
    for locale in locales:
        ...
        if locale == "ko":
            ...
            locale_rendered = locale_rendered.replace(
                "https://example.com/bare", "https://example.com/bare로"
            )
        ...
        if locale in ("fr", "ko") and mode == "bad-token-multiple-once" and calls == 1:
            locale_rendered = locale_rendered.replace("`literal`", "`translated`")
...
setup_repo docs
make_fake_agy
if run_hook "$WORK/repo" bad-token-multiple-once &&
  grep -qF 'docs/fr/page.mdx' "$WORK/agy-args" &&
  grep -qF 'docs/ko/page.mdx' "$WORK/agy-args"; then
  pass "one repair prompt reports every invalid locale target"
else
  fail "one repair prompt reports every invalid locale target" "$(cat "$WORK/output")"
fi
