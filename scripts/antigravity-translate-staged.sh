...
for action, source, expected_hash in requests:
    ...
    for target in targets:
        ...
        if not target_file.is_file() or target_file.is_symlink():
            raise SystemExit(f"REPAIRABLE: missing regular translation file: {target}")
        ...
        if not hash_match or hash_match.group(1) != expected_hash:
            raise SystemExit(f"REPAIRABLE: wrong or missing i18n.sourceHash in {target}")
        if not translator_match:
            raise SystemExit(f"REPAIRABLE: wrong or missing i18n.translator in {target}")
        if protected_tokens(raw) != source_tokens:
            raise SystemExit(f"REPAIRABLE: protected MDX/code/URL tokens changed in {target}")
PY
}
