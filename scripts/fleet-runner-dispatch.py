...
SPEC = importlib.util.spec_from_file_location("provision", ROOT / "provision-ephemeral-runners.py")
...
def state(repositories):
    try:
        ...
        raise PROVISION.ProvisionError(f"cannot read fleet dispatcher state: {exc}") from exc
    ...
    if not isinstance(value["cursor"], int) or not isinstance(cooldowns, dict) or set(cooldowns) != {"primary", "secondary"} or not all(isinstance(item, int) and item >= 0 for item in cooldowns.values()):
        raise PROVISION.ProvisionError("fleet dispatcher state is malformed")
...
def cache_path(path):
    return DISPATCH_ROOT / ("etag-" + hashlib.sha256(path.encode()).hexdigest() + ".json")
...
def get(github, path):
    ...
    valid = isinstance(cache, dict) and set(cache) == {"path", "etag", "body"} and cache["path"] == path and isinstance(cache["etag"], str) and cache["etag"]
    body, headers = github.request("GET", path, headers={"If-None-Match": cache["etag"]} if valid else {}, include_headers=True)
    if body is None:
        if not valid:
            raise PROVISION.ProvisionError("GitHub returned 304 without a valid fleet cache")
        return cache["body"]
    if not isinstance(body, dict):
        raise PROVISION.ProvisionError("GitHub fleet response is malformed")
    etag = next((value for key, value in headers.items() if key.lower() == "etag" and isinstance(value, str) and value), None)
    if etag:
        DISPATCH_ROOT.mkdir(parents=True, exist_ok=True)
        PROVISION.safe_write(destination, json.dumps({"path": path, "etag": etag, "body": body}, sort_keys=True), 0o600)
    return body


def valid_runs(response):
    runs = response.get("workflow_runs")
    if not isinstance(runs, list) or any(not isinstance(run, dict) or not isinstance(run.get("id"), int) for run in runs):
        ...
        try:
            for offset, repository in enumerate(ordered):
                ...
                for run in runs:
                    if requests >= policy.dispatcher.request_budget:
                        break
                    response = get(github, f"/repos/{repository}/actions/runs/{run['id']}/jobs?per_page=100")
                    requests += 1
                    jobs = response.get("jobs")
                    if not isinstance(jobs, list) or any(not isinstance(job, dict) for job in jobs):
                        raise PROVISION.ProvisionError("GitHub fleet job inventory is malformed")
                    trusted = any(PROVISION.successful_docker_trust_gate(job) for job in jobs)
                    for job in jobs:
                        labels = job.get("labels")
                        if job.get("status") != "queued" or not isinstance(labels, list) or not all(isinstance(label, str) for label in labels) or len(labels) != len(set(labels)):
                            continue
                        for profile in spec.profiles:
                            ...
                            print(f"[DISPATCH] repository={repository} profile={profile.name} unit={candidate.unit}")
                            break
                current["cursor"] = (cursor + offset + 1) % len(repositories)
                save(current)
        except controller_module.GitHubRateLimitError as exc:
            current["cooldowns"][exc.kind] = max(current["cooldowns"][exc.kind], exc.retry_at)
            save(current)
