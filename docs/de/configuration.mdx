...
### Antigravity-Automatisierungssteuerungen

Der Workflow `Configure Antigravity Controls` ändert die Organisationsvariablen, die den
Antigravity-Reviewer und -Übersetzer steuern. Jede Phase kann sicher erneut ausgeführt werden:
...
- `all` setzt beide Variablen auf `true` mit der Sichtbarkeit `all` erst nach Validierung des
  Review-Belegs des ausgewählten Piloten für den exakten HEAD und der Veröffentlichung der Übersetzung in 12 Locales.

Der Workflow verwendet das vorhandene Governance-Personal-Access-Token, begrenzte GitHub API-Wiederholungsversuche und
strukturierte Fortschritts-Heartbeats. Er erfordert keine GitHub App oder GitHub Enterprise-Funktionen.
