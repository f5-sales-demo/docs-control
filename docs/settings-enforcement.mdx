...
Detects whether the workflow is running on the source repo itself by comparing `managed_files.source_repo` against `github.repository`. If running on self, the `self_contexts` override is activated for Phase 4.

### Phase 2: Apply repository settings

Compares each key in the `repository` object against the repo's current settings via `GET /repos/{owner}/{repo}`. Builds a patch object containing only the keys that have drifted and applies it via `PATCH /repos/{owner}/{repo}`.
...
Compares the `topics` array against the repo's current topics via `GET /repos/{owner}/{repo}/topics`. Replaces via `PUT` if drifted.
