...
| Repository | Role |
|---|---|
| **docs-control** | Central governance hub -- repo settings, branch protection config, managed files manifest, reusable CI workflows, caller workflow templates, and downstream dispatch |
...
Content repos only need a `docs/` directory. The build container and workflow handle everything else.

## Data flow

When a template file changes in docs-control on `main`:

1. The [dispatch workflow](./dispatch/) fires and triggers enforcement in every downstream repo
...
| `workflows/` | Caller templates downstream repos install into `.github/workflows/` |
